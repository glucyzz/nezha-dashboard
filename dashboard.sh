...
check_cdn() {
  ...
  for PROXY_URL in "${GITHUB_PROXY[@]}"; do
    local PROXY_STATUS_CODE=$(wget --server-response --spider --quiet --timeout=3 --tries=1 ${PROXY_URL}https://github.com/fscarmen2/Argo-Nezha-Service-Container/raw/main/README.md 2>&1 | awk '/HTTP\//{last_field = $2} END {print last_field}')
...
dashboard_variables() {
  ...
  if [ -z "$DASHBOARD_VERSION" ]; then
    ...
    if [ "$VERSION_NUM" = "0.20.13" ]; then
      # 版本 = 0.20.13：从 glucyzz/nezha-dashboard 下载
      if wget -q --spider ${GH_PROXY}https://github.com/glucyzz/nezha-dashboard/releases/download/v0.20.13/dashboard-linux-$ARCH.zip 2>/dev/null; then
        { wget -qO $TEMP_DIR/dashboard.zip ${GH_PROXY}https://github.com/glucyzz/nezha-dashboard/releases/download/v0.20.13/dashboard-linux-$ARCH.zip >/dev/null 2>&1; }&
      else
